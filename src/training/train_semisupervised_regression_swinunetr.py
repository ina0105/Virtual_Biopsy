...
try:
    ...
    raise ImportError("MONAI is required for SwinUNETR. Install with: pip install 'monai[einops]>=1.3'")
...
DEFAULT_WORK    = DEFAULT_BASE / "semisup_swinunetr_work"

COMBINED_SUFFIX = "_biopsy_purity_absneg_innertumor_L3-4-11-12_enhancing"


class ChannelAdaptorModule(nn.Module):
    def __init__(self, model: nn.Module, pretrained_channels: int = 4, target_channels: int = 9):
        super().__init__()
        self.model = model
        ...
        first_conv = model.patch_embed.proj if hasattr(model, 'patch_embed') else None
        ...
        if first_conv is not None and hasattr(first_conv, 'in_channels'):
            self.adapter = nn.Conv3d(target_channels, pretrained_channels, kernel_size=1, bias=True)
            with torch.no_grad():
                self.adapter.weight.zero_()
                for out_ch in range(pretrained_channels):
                    in_ch = out_ch if target_channels >= pretrained_channels else (out_ch % target_channels)
                    self.adapter.weight[out_ch, in_ch, 0, 0, 0] = 1.0
...
def build_swinunetr(
    ...
    monai_hub_dir: Optional[Path] = None,
) -> nn.Module:

    model = SwinUNETR(
        in_channels=4,  # Base expects 4; we adapt to 9 channel input
    ...
    if pretrained:
        pretrain_url = "monai_swinunetr_brats_pretrain" if not use_v2 else "monai_swinunetr_brats_pretrain_v2"
        ...
        def _extract_state_dict(obj: Any) -> Dict[str, torch.Tensor]:
            ...
            if not isinstance(state, dict):
                raise TypeError("Checkpoint did not contain a usable state_dict")
        ...
        if pretrained_ckpt is not None:
            ...
            if ckpt_path.exists():
                try:
                    print(f"Loading BraTS pretrained SwinUNETR from checkpoint: {ckpt_path}")
                    ckpt_obj = torch.load(str(ckpt_path), map_location="cpu")
                    state = _extract_state_dict(ckpt_obj)
                    missing, unexpected = model.load_state_dict(state, strict=False)
                    print(
                        "Successfully loaded pretrained checkpoint "
                        f"(missing={len(missing)}, unexpected={len(unexpected)})"
                    )
                    loaded_pretrained = True
                except Exception as e:
                    print(f"Warning: Failed to load --swin-pretrained-ckpt ({ckpt_path}): {e}")
            else:
                print(f"Warning: --swin-pretrained-ckpt does not exist: {ckpt_path}")

        if not loaded_pretrained:
            hub_attempts: List[Tuple[str, str]] = []
            if monai_hub_dir is not None:
                local_hub = Path(monai_hub_dir)
                if (local_hub / "hubconf.py").exists():
                    hub_attempts.append(("local", str(local_hub)))
                else:
                    print(
                        f"Info: --monai-hub-dir set but hubconf.py not found at {local_hub / 'hubconf.py'}"
                    )
            hub_attempts.append(("github", "Project-MONAI/MONAI"))

            for source, repo_or_dir in hub_attempts:
                try:
                    if source == "local":
                        print(f"Loading BraTS pretrained SwinUNETR from local hub dir: {repo_or_dir}")
                        ...
                        print("Loading BraTS pretrained SwinUNETR from torch hub (GitHub)...")
                        pretrained_model = torch.hub.load(
                            repo_or_dir,
                            model=pretrain_url,
                            pretrained=True,
                    ...
                    state = _extract_state_dict(pretrained_model)
                    missing, unexpected = model.load_state_dict(state, strict=False)
                    print(
                        "Successfully loaded pretrained weights "
                        f"via {source} (missing={len(missing)}, unexpected={len(unexpected)})"
                    )
                    loaded_pretrained = True
                    break
                except Exception as e:
                    print(f"Warning: Could not load pretrained weights via {source}: {e}")

        if not loaded_pretrained:
            print("Training from random initialization instead")

    model = ChannelAdaptorModule(model, pretrained_channels=4, target_channels=in_channels)
...
def _load_nifti_data(path: Path, max_retries: int = 3, retry_wait_s: float = 0.5) -> np.ndarray:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                print(f"WARNING: failed reading {path} (attempt {attempt}/{max_retries}): {e}")
                time.sleep(retry_wait_s)
    raise RuntimeError(f"Failed to read NIfTI after {max_retries} attempts: {path} ({last_err})")
...
def load_all_patients(
    ...
    """Return (fully_labeled, partially_labeled) patient records."""
    ...
    for pid, pinfo in patients_info.items():
        ...
        try:
            ...
            lbl_data = np.clip(_load_nifti_data(lbl_path), 0.0, 1.0)
        except Exception as e:  # noqa: BLE001
            print(f"WARNING: failed loading patient {pid}, skipping. Reason: {e}")
            continue

        ann_mask = build_annotation_mask(lbl_data, seg_path, absneg_labels)

        target = lbl_data[None]
...
def masked_regression_loss(
    ...
    """Regression loss only at annotated voxels, optionally biopsy-ROI weighted."""
    n = ann_mask.sum()
    if n == 0:
...
def train_fold(
    ...
    print(f"Building SwinUNETR with {in_channels} input channels, pretrained={args.pretrained}")
    model = build_swinunetr(
        in_channels=in_channels,
        out_channels=1,
        pretrained=args.pretrained,
        feature_size=args.feature_size,
        use_v2=args.swinunetr_v2,
        pretrained_ckpt=args.swin_pretrained_ckpt,
        monai_hub_dir=args.monai_hub_dir,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for imgs, tgts, masks, wmaps in dl_val:
                ...
                def divisible_pad(x, divisor=32):
                    *_, d, h, w = x.shape
                ...
                imgs_padded, (pad_d, pad_h, pad_w) = divisible_pad(imgs, divisor=32)
                preds = model(imgs_padded)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({"model_state": model.state_dict(), "val_loss": val_loss}, fold_dir / "best_model.pt")
    ...
    if bcsv.exists() and ckpt_path.exists():
        ...
        for rec in val_records:
            with torch.no_grad():
                img_tensor = torch.from_numpy(rec.image[None]).to(device)
                *_, d, h, w = img_tensor.shape
                pad_d = (32 - d % 32) % 32
                pad_h = (32 - h % 32) % 32
                pad_w = (32 - w % 32) % 32
                img_padded = F.pad(img_tensor, (0, pad_w, 0, pad_h, 0, pad_d))
                pred_padded = model(img_padded)
                if pad_d > 0:
                    pred_padded = pred_padded[:, :, :-pad_d, :, :]
                if pad_h > 0:
                    pred_padded = pred_padded[:, :, :, :-pad_h, :]
                if pad_w > 0:
                    pred_padded = pred_padded[:, :, :, :, :-pad_w]
                pred_vol = pred_padded.cpu().numpy()[0, 0]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Semi-supervised 3D SwinUNETR purity regression (dataset_final)")
    ...
    p.add_argument("--pretrained",     action="store_true",
                   help="Load BraTS pretrained weights from MONAI")
    p.add_argument("--swin-pretrained-ckpt", type=Path, default=None,
                   help="Optional local checkpoint path to load SwinUNETR pretrained weights from")
    p.add_argument("--monai-hub-dir", type=Path, default=None,
                   help="Optional local clone of Project-MONAI/MONAI containing hubconf.py for torch.hub source=local")
    p.add_argument("--feature-size",   type=int,   default=48,
                   help="Base feature dimension for SwinUNETR")
    p.add_argument("--swinunetr-v2",   action="store_true",
                   help="Use SwinUNETR v2 (slightly improved version)")
    # Semi-supervised
...
def main() -> None:
    ...
    print(f"Model: SwinUNETR (pretrained={args.pretrained}, feature_size={args.feature_size}, v2={args.swinunetr_v2})")
